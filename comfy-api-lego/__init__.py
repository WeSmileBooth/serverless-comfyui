...
app = modal.App("comfy-api-lego")

# Add the workflow file to the image
workflow_path = pathlib.Path(__file__).parent / "workflow_api.json"
...
class ComfyUI:
    ...
    def infer(self, input: InferModel):
        try:
            ...
            data = json.dumps(
                {"prompt": workflow, "client_id": input.session_id}
            ).encode("utf-8")
                ...
                if isinstance(out, str):
                    ...
                    if message["type"] == "executing":
                        ...
                        if (
                            ...
                            doc_ref.update(
                                {"progress": data["value"], "status": "pending"}
                            )
                else:
                    if (
...
def api():
    ...
    ComfyUI = modal.Cls.lookup("comfy-api-lego", "ComfyUI")
    comfyui = ComfyUI()
